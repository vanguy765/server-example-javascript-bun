...
const PHONE_NUMBER_ID = "800187f4-d205-401b-a90e-82dba423e4a1"; // Replace
const PHONE_NUMBER = process.env.TWILIO_PHONE_NUMBER; // Replace
// const ASSISTANT_ID = "vapi_assistant_id"; // Replace
...
app.post("/", async (c) => {
  ...
  let domain = c.req.header("host") || "localhost";
  ...
  // Get the tenant using the domain`

  // ------------------------------------------------------------------

  const tenantRepository = await getSafeRepository("tenants");
  const tenant = await tenantRepository
    .query()
    .select("*")
    .eq("domain", domain)
    .maybeSingle();
  // console.log("Tenant retrieved:", tenant);
  if (!tenant || !tenant.data) {
    return c.json({ message: "Tenant not found for the provided domain" }, 404);
  }

  // ------------------------------------------------------------------

  // Get the agent (phoneNumberId for Vapi and smsNumber for Twilio)
  // using the tenantId and the lastPart of the path
  // const agentRepository = await getSafeRepository("agents");
  // const agentData = await agentRepository
  //   .query()
  //   .select("*")
  //   .eq("tenant_id", TENANT_ID)
  //   .eq("name", lastPart)
  //   .maybeSingle();
  // // console.log("Agent retrieved:", agentData);
  // if (!agentData || !agentData.data) {
  //   return c.json(
  //     { message: "Agent not found for the provided tenant and path" },
  //     404
  //   );
  // }
